...
async function most_hated(){
  hatest.value = []
  for (const address_count of top_ten_recent.value){
...
//   const key="num"
//   return top_ten_recent.value.sort(function(a,b){
//     var x=a[key];
//     var y=b[key];
...
<template>
  ...
  <div class=wallet v-if="states.isLogged">
    ...
    <button @click="getMyMessage">my msg: {{a}}</button>
   </div>
  <div class=wallet>
    <ul>
    <li v-for="todo in top_ten_recent" :key="todo.num">
  ...
  <div class=wallet>
    <ul>
    <li v-for="todo in hatest" :key="todo.num">
      {{ todo.address}} {{ todo.num}} 
    </li>
    </ul>  
  </div>
